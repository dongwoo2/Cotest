...
public class Study240114_1 {
        ...
        while (flag) {
            ...
            if(ch <= 5) {
                if(people[peoplecnt] < 1000) {
                    ...
                    menuchoice(ch,people,peoplecnt,menuprice);
                }
            } else if (ch == 6) { // 돈 갚기
    ...
    // 리턴되는 값이 결제한 사람의 결제하고 남은돈, 음식갯수 // 총 정보가 몇 번째 사람, 남은 돈, 음식 갯수
    public static int[] menuchoice(int ch, int people[],int peoplecnt, int menuprice[]) { // 몇 번째 사람인지 체크해주어야 할 듯
        ch -= 1;
        ...
        if(people[peoplecnt] >= menuprice[ch]) {
            ...
            ch = sc.nextInt();
            if(ch == 1){
                borrowmoney(peoplecnt,people);
            }
        }
           return new int[]{people[peoplecnt],foodcnt[ch]};
    }

    public static int borrowmoney(int peoplecnt,int people[]){ //리턴 되어야하는 값은 돈을 빌려준 사람이 빌려준만큼 빠진돈과 빌린 사람이 가진 돈
        peoplecnt++;
        int money = 0;
        System.out.println(peoplecnt + "번 째 손님이 빌려줄 수 있는 돈은" + people[peoplecnt] + "입니다.");
        System.out.println("얼마를 빌리시겠습니까?");
        money = sc.nextInt();

        if(money > people[peoplecnt]) {
            System.out.println(peoplecnt + "번 째 손님이 가진 돈보다 많습니다");
        } else if (money < people[peoplecnt]) {
