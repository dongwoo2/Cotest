...
public class Study240114_1 {
    ...
    public static void solution() {
        ...
        int firstmoney[] = new int[n]; //현재 가지고 있는 돈 담아줄 변수
        int t = 0;
        int foodch = 0;
        int arr[] = new int[50];
        int brr[] = new int[50];
        ...
        while (flag) {
            System.out.println(peoplecnt+1 + "번째 손님입니다.");
            //peoplecnt--;
            borrowpeople = peoplecnt+1;
            if(se == 0) { //손님 제어 안했을 시
                firstmoney[peoplecnt] = people[peoplecnt];
            }
            borrowcount = 0;
            ...
            flag2 = true;
            while(flag2) {
                menuname(se); // 손님 제어 후 다음 손님으로 돌아가기 생겨서 인자 전달
                ch = sc.nextInt();
                flag3 = true;
                if (ch <= 5) {
                    if (people[peoplecnt] < 1000) { // 소지금 부족 할 경우
                        System.out.println("소지금이 부족합니다. 어떤 메뉴도 고를 수 없습니다.");
                        System.out.println("1.다시 선택한다 2.다음 손님으로 넘어간다 3.돈을 빌린다");
                        ch = sc.nextInt();
                        if(ch != 1 && se != 0) {
                    ...
                    refund(foodcnt,ch,people,peoplecnt,menuprice);
                    //환불
                } else if(ch == 10) {
                    //종료
                    receiptlist = receiptinsert(firstmoney, foodcnt, people, peopledebt, debtwpay, borrowpe, peoplecnt, receiptlist);
                    ...
                    flag2 = false;
                    flag = false;
                    printReceipt(receiptlist,peoplecnt,menuprice);
                }
    ...
    public static void menuname(int se) { // 메뉴
        if (se == 0) {
    ...
    public static int[] borrowmoney(int peoplecnt, int people[], int borrowpeople, int peopledebt[], int borrowpe[]) { //리턴 되어야하는 값은 돈을 빌려준 사람이 빌려준만큼 빠진돈과 빌린 사람이 가진 돈
        ...
        System.out.println((borrowpeople + 1) + "번 째 손님이 빌려줄 수 있는 돈은" + people[borrowpeople] + "입니다.");
        System.out.println("얼마를 빌리시겠습니까?");
        money = sc.nextInt();

        if (money > people[borrowpeople]) {
            System.out.println((borrowpeople + 1) + "번 째 손님이 가진 돈보다 많습니다");
            ...
            System.out.println(money + "원 을 빌렸습니다.");
    ...
    public static int[] receiptinsert(int firstmoney[], int foodcnt[], int people[], int peopledebt[], int debtpay[], int borrowpe[], int peoplecnt, int receiptlist[]) { // 영수증에 넣어주는 함수
        if(peoplecnt != 0) {
            for(int i = peoplecnt * 11; i < peoplecnt * 11 + 1; i++) {
                receiptlist[i] = firstmoney[peoplecnt];
                receiptlist[i+1] = foodcnt[(peoplecnt * 5)];
                receiptlist[i+2] = foodcnt[(peoplecnt * 5) + 1];
                receiptlist[i+3] = foodcnt[(peoplecnt * 5) + 2];
                receiptlist[i+4] = foodcnt[(peoplecnt * 5) + 3];
                receiptlist[i+5] = foodcnt[(peoplecnt * 5) + 4];
            ...
            for(int i = 0; i < peoplecnt * 11+1; i +=11) {
                receiptlist[i] = firstmoney[peoplecnt];
                receiptlist[i+1] = foodcnt[(peoplecnt * 5)];
                receiptlist[i+2] = foodcnt[(peoplecnt * 5) + 1];
                receiptlist[i+3] = foodcnt[(peoplecnt * 5) + 2];
