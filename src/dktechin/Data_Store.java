...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException {
            ...
            public void run() {
                while (true) {
                    try {
                        ...
                        // 데이터 저장소에 저장
                        Data_Store.getInstance().store(entry.getKey(), entry.getValue());
                        // 목표 개수에 도달하면 종료
                        if (Data_Store.getInstance().size() >= TARGET_COUNT) break;
                    } catch (InterruptedException e) {
        ...
        System.out.println("Generated " + Data_Store.getInstance().size() + " unique strings.");
    }
...
class Data_Store {
    ...
    // 싱글톤 인스턴스 반환 메소드
    public static synchronized Data_Store getInstance() {
        if (instance == null) {
